...
> **⚠ 严格禁止**：不要编写 Python/Shell 脚本来批量分类或处理文件。分类（Step 3）必须由你（Agent/subagent）通过阅读每份文档的实际内容来完成——图片和扫描件用视觉能力，文字 PDF 用文本提取后阅读。不能用关键词匹配、正则表达式、文件名推断或任何硬编码规则代替 LLM 理解。`document-taxonomy.md` 中的类别描述是给你理解用的参考，不是给代码用的数据源。
