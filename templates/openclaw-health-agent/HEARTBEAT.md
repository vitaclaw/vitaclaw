...
- 行为计划、执行障碍、周报/月报待阅读也属于 heartbeat 负责推进的用户可感知产物

## 推荐脚本入口

- 巡检：`python3 scripts/run_health_heartbeat.py`
- 任务管理：`python3 scripts/manage_health_tasks.py list|complete|snooze|reopen`
- 旗舰场景：`python3 scripts/run_hypertension_daily_copilot.py` / `run_diabetes_control_hub.py` / `run_annual_checkup_advisor.py`
