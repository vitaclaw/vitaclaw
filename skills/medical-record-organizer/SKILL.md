...
metadata: {"openclaw":{"emoji":"🏥","category":"health-records","requires":{"bins":["python3"],"anyBins":["unrar","unar"]},"install":[{"kind":"uv","package":"pdfplumber"},{"kind":"uv","package":"rarfile"},{"kind":"uv","package":"pillow"},{"kind":"uv","package":"python-docx"},{"kind":"uv","package":"openpyxl"},{"kind":"uv","package":"paddlepaddle"},{"kind":"uv","package":"paddleocr"},{"kind":"uv","package":"paddlenlp"},{"kind":"uv","package":"opencv-python"},{"kind":"uv","package":"PyMuPDF"}]}}
---
...
```
输入文件 → 检测类型 → 提取文本 → 分类文档 → [图片?] 隐私遮挡 → 提取关键字段 → 复制并重命名 → 更新索引 → 更新 profile → [触发?] 更新当前状态 → 同步关键指标到 workspace
```

> **严格顺序执行**：Agent 必须按 Step 0 → Step 1 → Step 2 → … → Step 10 → Step 11 的编号顺序依次执行。每个 Step 完成后方可进入下一个 Step，不得跳过或乱序。批量模式下 subagent 内部同样遵守 Step 2 → Step 3 → Step 4 → Step 5 → Step 6 的顺序。
...
6. **禁止跳步或乱序执行**：必须按 Step 编号顺序（0 → 1 → 2 → … → 10 → 11）依次执行，前一个 Step 完成后才能进入下一个 Step。批量模式下每个 subagent 也必须按 Step 2 → 3 → 4 → 5 → 6 的顺序执行。
...
## Step 0.5 — 身份检测：这份报告是谁的？

在初始化患者目录之前，先判断待归档文档属于哪位患者。

### 执行步骤

1. **快速预读文档**：使用 Step 2 的方法提取或查看文档中的患者基本信息（姓名、性别、年龄），但不需要完成完整分类。

2. **读取当前 workspace 的健康档案**：读取 `memory/health/_health-profile.md` 中的当事人信息（姓名/代号、性别、年龄）。

3. **对比判断**：
   - 如果文档中的患者信息与当前健康档案匹配（同一人）→ 直接进入 Step 1
   - 如果文档中的患者信息与当前健康档案**不匹配**（不同人）→ 提示用户：

     > "检测到这份报告的患者是 [文档中的姓名/信息]，和当前档案（[当前档案信息]）不是同一人。
     > 1. 为 [文档中的姓名] 创建新的健康档案
     > 2. 仍然归入当前档案
     > 请选择（1/2）"

   - 如果文档中无法提取患者信息 → 直接进入 Step 1（按常规流程询问患者代号）

4. **如果用户选择 1（创建新档案）**：
   - 运行 `$PYTHON scripts/init_health_workspace.py [新 workspace 路径] --template health-family-agent` 为该家人创建 workspace
   - 在新 workspace 的 `memory/health/files/patient-archive-link.json` 中写入即将创建的患者 ID
   - 在当前 workspace 的 `_health-profile.md` 末尾追加家人索引：
     ```markdown
     ## 家人档案索引
     - [姓名/代号]: workspace 路径 [新 workspace 路径]
     ```
   - 然后进入 Step 1，使用新档案的上下文继续归档
...
```bash
$PYTHON scripts/init_patient.py "[患者代号或姓名]"
```
...
```bash
$PYTHON scripts/extract_pdf.py "[文件路径]"
```
...
```bash
$PYTHON scripts/extract_excel.py "[文件路径]"
```
...
```bash
$PYTHON scripts/unpack_archive.py "[压缩包路径]"
```
...
## Step 4 — 图片/扫描件隐私遮挡（OCR + 条码检测 + LLM 增强方案）

**触发条件**：在以下任一条件满足时执行：
1. 输入为图片（`.jpg`/`.png`/`.bmp`/`.webp`/`.heic`/`.heif`）
2. 输入为扫描件 PDF（Step 2b 中 `image_only` 为 `true` 的 PDF）

**脱敏流水线**（`redact_ocr.py` 内置）：

```
输入 → [HEIC转换] → PaddleOCR 文字检测
                   → 条码/二维码检测（OpenCV + pyzbar）
       → PII 分类（正则 + PaddleNLP NER）
       → LLM 补充识别（可选，14 类隐私，覆盖 NER 的盲区）
       → 标签-值分离（只遮值不遮标签）
       → 黑色多边形遮挡文字 PII + 条码区域
       → 输出遮挡图
```
...
**增强能力**（相比原有方案）：
- **条码/二维码检测**：自动检测并遮挡图片中的一维条码和二维码（含患者编号等隐私信息）
- **LLM 补充识别**：在正则+NER 之后，将剩余未标记文本发给本地 LLM（需配置 `LLM_API_BASE`/`LLM_MODEL` 环境变量），LLM 基于 14 类隐私规则进行二次筛查。LLM 不可用时自动跳过，不影响基础流程
- **HEIC/HEIF 支持**：自动转换为 JPG 后处理
- **扫描件 PDF 原生支持**：逐页渲染为图片 → 每页独立脱敏 → 重新合成 PDF

**对于扫描件 PDF**：`redact_ocr.py` 现已原生支持 PDF 输入，自动按 DPI=200 渲染每页、逐页脱敏、重新合成脱敏后的 PDF 文件。如果 PyMuPDF 未安装导致 PDF 处理失败，则将该 PDF 归入 `10_原始文件/原始未遮挡/`，明确告知用户”该扫描件 PDF 需要手动脱敏后才能进入分类目录”。

其他文件类型（有文字层的 PDF、Word、Excel、文本）跳过此步。

**时序约束**：必须在 Step 3（分类）之后、Step 5（提取关键字段）之前执行。Step 6 应复制 `_redacted` 版本而非原始文件。

**隐私闸门**：隐私模式开启时，图片类文件如果没有成功生成 `_redacted` 版本，**禁止**进入分类目录。此时只能保留 `10_原始文件/原始未遮挡/` 备份，并明确告知用户”归档被隐私闸门阻断”。

**LLM 环境变量**（可选，不配置则仅使用 正则+NER 基础流程）：
- `LLM_API_BASE` — LLM API 地址（默认 `http://localhost:8000/v1`）
- `LLM_API_KEY` — API Key
- `LLM_MODEL` — 模型名称（为空则跳过 LLM 补充）
...
```bash
$PYTHON scripts/ensure_redaction_runtime.py --require-ready
```

如果返回 `success=false`，优先尝试自动安装依赖：

```bash
$PYTHON scripts/ensure_redaction_runtime.py --require-ready --auto-install
```
...
```bash
$PYTHON scripts/redact_ocr.py “[原始文件路径]” --output “[输出路径_redacted.ext]”
```

对于**扫描件 PDF**输入：

```bash
$PYTHON scripts/redact_ocr.py “[原始PDF路径]” --output “[输出路径_redacted.pdf]”
```

脚本自动完成：HEIC 转换 → PaddleOCR 文字检测 → 条码/二维码检测 → PII 分类（正则 + NER + LLM 补充）→ 标签-值分离（只遮值不遮标签）→ 输出遮挡图/PDF。

如需跳过 LLM 补充（例如本地没有 LLM 服务），可加 `--no-llm` 参数。

**输出**：
脚本输出的遮挡图/PDF 不需要再进行任何额外脱敏处理。
...
**禁止手写 `cp` 到分类目录。** Step 6 必须统一调用安全复制脚本，它会自动：
- 将原始未遮挡文件备份到 `10_原始文件/原始未遮挡/`
- 在隐私模式 ON 且输入为图片时，自动触发 `redact_ocr.py`
- 如果脱敏失败，则阻断分类归档（fail-closed）

统一命令如下：

```bash
$PYTHON scripts/copy_patient_document.py \
  "[原始文件路径]" \
...
## Step 11 — 同步关键指标到 workspace 健康日记

**执行主体**：由你（Agent）直接完成。不得编写脚本。

> **批量模式下**：所有文件处理完毕后只执行**一次**同步。

归档完成后，将本次提取的关键指标同步到当前 workspace 的 `memory/health/` 目录，使日常健康追踪系统能感知到新归档的数据。

### 执行步骤

1. **确认 workspace 路径**：读取环境变量 `OPENCLAW_WORKSPACE` 或 `VITACLAW_WORKSPACE`，如果未设置则使用 `~/openclaw/workspace-health-main`。确认 `memory/health/` 目录存在。如果不存在，跳过此步并提示用户"未检测到健康日记 workspace，归档已完成但指标未同步到日常追踪"。

2. **更新对应 item 文件**：根据 Step 5 提取的 `doc_type` 和 `key_values`，将关键数值写入对应的 `memory/health/items/*.md`：

   | 文档类型 | 目标 item 文件 | 写入内容 |
   |---------|---------------|---------|
   | 血常规/血液检查 | `blood-pressure.md`、`blood-sugar.md` 等 | 对应数值 |
   | 肿瘤标志物 | `tumor-markers.md` | CEA、CA-199、AFP 等 |
   | 肝功能 | `liver-function.md` | ALT、AST、GGT 等 |
   | 肾功能 | `kidney-function.md` | 肌酐、尿素氮、eGFR 等 |
   | 血脂 | `blood-lipids.md` | TC、TG、LDL-C、HDL-C 等 |
   | 影像报告 | 不写 item，仅写 daily | 摘要 |

   写入方式遵循 `health-memory` SKILL.md 的写入协议：在 item 文件的 `## History` 表格中追加新行（按时间倒序），更新 `## Recent Status` 段落。

3. **更新今日 daily 文件**：在 `memory/health/daily/YYYY-MM-DD.md` 中追加一个段落：

   ```markdown
   ## 病历归档 [medical-record-organizer . HH:MM]

   归档了 [doc_type]（[date]，[hospital]）。
   关键发现：[summary]
   [如有异常项] 异常项：[abnormal_items]
   ```

4. **触发患者档案同步**（可选）：如果 `memory/health/files/patient-archive-link.json` 存在且关联了当前患者，运行：

   ```bash
   $PYTHON scripts/sync_patient_archive.py
   ```
...
核心原则：除非用户明确说”关闭隐私模式”，否则始终启用。目录必须使用匿名ID，文件名和索引内容禁止出现 PII。图片/扫描件的 PII 遮挡由 Step 4 的 `redact_ocr.py` 自动完成（含条码/二维码检测 + LLM 补充识别）。
...
```bash
$PYTHON scripts/repair_patient_images.py --patient-dir "$PATIENT_DIR"
```
...
- 如果隐私模式开启且图片脱敏失败，必须停止流程。只允许保留 `10_原始文件/原始未遮挡/` 备份，并明确告知用户“分类归档被隐私闸门阻断，原因是脱敏运行时未就绪或执行失败”。
- 所有文件操作使用 UTF-8 编码。
