...
```bash
python3 ~/.openclaw/skills/medical-record-intake/scripts/init_patient.py "[患者姓名]"
```
...
```bash
python3 ~/.openclaw/skills/medical-record-intake/scripts/extract_pdf.py "[文件路径]"
```
...
```bash
python3 ~/.openclaw/skills/medical-record-intake/scripts/unpack_archive.py "[压缩包路径]"
```

脚本返回 JSON，包含 `tmp_dir`（临时目录）和 `files`（文件列表）。

2. **逐文件处理**：对 `files` 列表中的每个文件，依次执行：
   - Step 2（检测类型并提取文本）
   - Step 3（分类）
   - Step 4（提取关键字段）
   - Step 5（写入 Markdown 归档文件）

3. **批量更新索引**：所有文件处理完毕后，**仅执行一次** Step 6（更新 INDEX.md）和 Step 7（更新 timeline.md）。
...
```
~/.openclaw/skills/medical-record-intake/assets/document-taxonomy.md
```
...
```bash
cp ~/.openclaw/skills/medical-record-intake/assets/templates/INDEX-template.md $PATIENT_DIR/INDEX.md
```
